...
const PAYMENT_TYPE_MAP: Partial<
  Record<FiatPaymentType, FiatPaymentType | undefined>
> = {
  ach: 'ach',
  applepay: 'credit',
  credit: 'credit',
  ...
  makeProvider: async (
    ...
    const out: FiatProvider = {
      ...
      getSupportedAssets: async ({
        ...
        if (
          Object.keys(assetMap.crypto).length === 0 ||
          isDailyCheckDue(lastChecked)
        ) {
          ...
          for (const currency of moonpayCurrencies) {
            ...
            if (currency.type === 'crypto') {
              ...
              if (currency.isSuspended === true) continue
      ...
      getQuote: async (
        ...
        const moonpayCurrency = tokens.find(
          token => token.tokenId === params.tokenId
        )
        const cryptoCurrencyObj = asMoonpayCurrency(moonpayCurrency?.otherInfo)
        const fiatCurrencyObj = asMoonpayCurrency(
          assetMap.fiat[params.fiatCurrencyCode]
        )
        if (cryptoCurrencyObj == null || fiatCurrencyObj == null)
          throw new Error('Moonpay could not query supported currencies')
        ...
        const paymentQuote: FiatProviderQuote = {
          ...
          approveQuote: async (
            ...
            if (direction === 'buy') {
              ...
              const handleBuyDeeplink = (link: unknown): void => {
              ...
              const openWebView = async (): Promise<void> => {
                  ...
                  if (uri.startsWith(RETURN_URL_PAYMENT)) {
                    ...
                    try {
                          ...
                          spendTargets: [
                            {
                              ...
                              otherParams:
                                depositWalletAddressTag == null
                                  ? undefined
                                  : {
                                      uniqueIdentifier: depositWalletAddressTag
                                    }
                            }
                          ]
                        },
                      ...
                      if (params.tokenId != null) {
                        const paramsSave: SaveTxActionParams = {
                          walletId: coreWallet.id,
                          tokenId: params.tokenId,
                          txid: tx.txid,
                          savedAction,
                        ...
                        await showUi.saveTxAction(paramsSave)
                      }
                ...
                const onUrlChange = (uri: string): void => {
